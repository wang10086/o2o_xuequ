    ...
    <aside class="right-side">
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li><a href="<?php echo U('News/pic_index',array('type'=>$type));?>"><?php echo ($pagename); ?>管理</a></li>
            </ol>
        </section>
                    ...
                    <div class="box">
                        <div class="box-header">
                            ...
                            <div class="pull-right box-tools">
                                ...
                                <?php if(rolemenu('News/pic_add')): ?><a href="<?php echo U('News/pic_add',array('kind'=>$kind));?>" style="color:#ffffff;" class="btn btn-danger btn-sm"><i class="fa fa-plus"></i> 发布文章</a><?php endif; ?>
                            </div>
                        </div><!-- /.box-header -->
                        <div class="box-body" id="piclist">
                            <table class="table table-bordered dataTable fontmini" id="tablelist">
                                <tr role="row" class="orders" >
                                    ...
                                    <?php if(rolemenu('News/pic_edit')): ?><th width="60" class="taskOptions">编辑</th><?php endif; ?>
                                    <?php if($type==1){ ?>
                                        <?php if(rolemenu('Delete/del_pic_news')): ?><th width="60" class="taskOptions">删除</th><?php endif; ?>
                                    <?php }else{ ?>
                                ...
                                <?php if(is_array($infolist)): foreach($infolist as $key=>$row): ?><tr>
                                        ...
                                        <td style="line-height:24px;">
                                            <?php echo ($row["pic"]); ?>
                                            <?php
 $con = C('TMPL_PARSE_STRING'); if($kind==1){ $url = $con['__HTTP_URL__'].'index.php?m=Main&c=News&a=detail&id='.$row['id']; }else if($kind==2){ $url = $con['__HTTP_URL__'].'index.php?m=Main&c=Collection&a=detail&id='.$row['id']; }else{ $url = 'javascript:;'; } ?>
                                        ...
                                        <?php if(rolemenu('News/pic_edit')): ?><td class="taskOptions"  style="line-height:24px;">
                                                <a href="<?php echo U('News/pic_edit',array('id'=>$row['id'],'kind'=>$kind));?>" title="修改" class="btn btn-info btn-smsm"><i class="fa fa-pencil"></i></a>
                                            </td><?php endif; ?>

                                        <?php if(rolemenu('News/del_pic_news')): ?><td class="taskOptions" style="line-height:24px;">
                                                <a onClick="javascript:ConfirmDel('<?php echo U('News/del_pic_news',array('id'=>$row['id']));?>');" title="删除" class="btn btn-warning btn-smsm"><i class="fa fa-times"></i></a>
                                            </td><?php endif; ?>
...
<div id="searchtext">
    <form method="get" action="<?php echo U('News/index');?>" id="searchfrom" name="myform">
