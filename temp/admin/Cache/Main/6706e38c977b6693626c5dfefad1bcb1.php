    ...
    <aside class="right-side">
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li><a href="<?php echo U('Union/index',array('type'=>$type));?>"><?php echo ($pagename); ?>管理</a></li>
            </ol>
        </section>
                    ...
                    <div class="box">
                        <div class="box-header">
                            ...
                            <div class="pull-right box-tools">
                                ...
                                <?php if(rolemenu('Union/add')): ?><a href="<?php echo U('Union/add',array('kind'=>$kind));?>" style="color:#ffffff;" class="btn btn-danger btn-sm"><i class="fa fa-plus"></i> 发布文章</a><?php endif; ?>
                            </div>
                        </div><!-- /.box-header -->
                        <div class="box-body" id="piclist">
                            <table class="table table-bordered dataTable fontmini" id="tablelist">
                                <tr role="row" class="orders" >
                                    ...
                                    <?php if(rolemenu('Union/edit')): ?><th width="60" class="taskOptions">编辑</th><?php endif; ?>
                                    <?php if($type==1){ ?>
                                ...
                                <?php if(is_array($infolist)): foreach($infolist as $key=>$row): ?><tr>
                                        ...
                                        <td style="line-height:24px;">
                                            <?php echo ($row["pic"]); ?>
                                            <?php
 $con = C('TMPL_PARSE_STRING'); if($kind==1){ $url = $con['__HTTP_URL__'].'index.php?m=Main&c=Union&a=detail&id='.$row['id']; }else if($kind==2){ $url = $con['__HTTP_URL__'].'index.php?m=Main&c=Collection&a=detail&id='.$row['id']; }else{ $url = 'javascript:;'; } ?>
                                            <a href="<?php echo ($url); ?>" target="_blank"><?php echo ($row["title"]); ?></a>

                                        </td>
                                        <td style="line-height:24px;"><?php echo get_kind($row['col']);?></td>
                                        ...
                                        <?php if(rolemenu('Union/edit')): ?><td class="taskOptions"  style="line-height:24px;">
                                                <a href="<?php echo U('Union/edit',array('id'=>$row['id'],'kind'=>$kind));?>" title="修改" class="btn btn-info btn-smsm"><i class="fa fa-pencil"></i></a>
                                            </td><?php endif; ?>

                                        <?php if(rolemenu('Union/del_news')): ?><td class="taskOptions" style="line-height:24px;">
                                                <a onClick="javascript:ConfirmDel('<?php echo U('Union/del_news',array('id'=>$row['id']));?>');" title="删除" class="btn btn-warning btn-smsm"><i class="fa fa-times"></i></a>
                                            </td><?php endif; ?>
...
<div id="searchtext">
    <form method="get" action="<?php echo U('Union/index');?>" id="searchfrom" name="myform">
        <input type="hidden" name="m" value="Main">
        <input type="hidden" name="c" value="Union">
