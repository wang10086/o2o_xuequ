    ...
    <aside class="right-side">
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li><a href="<?php echo U('Res/index');?>">专家管理</a></li>
            </ol>
        </section>
                    ...
                    <div class="box">
                        <div class="box-header">
                            ...
                            <div class="pull-right box-tools">
                                ...
                                <?php if($type == 2): ?><button onClick="javascript:window.location.href='<?php echo U('Res/res_upd',array('type'=>$type));?>';" class="btn btn-danger btn-sm"><i class="fa fa-plus"></i> 新增资源</button>
                                <?php else: ?>
                                    <button onClick="javascript:window.location.href='<?php echo U('Res/res_edit',array('type'=>$type));?>';" class="btn btn-danger btn-sm"><i class="fa fa-plus"></i> 新增资源</button><?php endif; ?>
                            </div>
                        </div><!-- /.box-header -->
                        <div class="box-body">

                            <div class="btn-group" id="catfont">
                                <a href="<?php echo U('Res/index');?>" class="btn <?php if(!$type){ echo 'btn-info';}else{ echo 'btn-default'; } ?>">全部</a>
                                <?php if(is_array($kind)): foreach($kind as $k=>$v): ?><a href="<?php echo U('Res/index',array('type'=>$k));?>" class="btn <?php if($k==$type){ echo 'btn-info';}else{ echo 'btn-default'; } ?>"><?php echo ($v); ?></a><?php endforeach; endif; ?>
                            </div>
                            ...
                            <table class="table table-bordered dataTable fontmini" id="tablelist" style="margin-top:10px;">
                                <tr role="row" class="orders" >
                                    ...
                                    <?php if(rolemenu(array('Res/expert_edit'))): ?><th width="60" class="taskOptions">编辑</th><?php endif; ?>
                                    <?php if(rolemenu('Res/expert_del')): ?><th width="60" class="taskOptions">删除</th><?php endif; ?>
                                </tr>
                                <?php if(is_array($datalist)): foreach($datalist as $key=>$row): ?><tr>
                                        ...
                                        <!--<?php if(rolemenu(array('Res/res_edit'))): ?>-->
                                        <td class="taskOptions">
                                            <button onClick="javascript:window.location.href='<?php echo U('Res/update',array('id'=>$row['id'],'type'=>$type));?>';" title="修改" class="btn btn-info btn-smsm"><i class="fa fa-pencil"></i></button>
                                        </td>
                                        <!--<if condition="$type eq 2">
                                            <td class="taskOptions">
                                                <button onClick="javascript:window.location.href='<?php echo U('Res/res_upd',array('id'=>$row['id'],'type'=>$type));?>';" title="修改" class="btn btn-info btn-smsm"><i class="fa fa-pencil"></i></button>
                                            </td>
                                        <?php else: ?>
                                            <td class="taskOptions">
                                                <button onClick="javascript:window.location.href='<?php echo U('Res/res_edit',array('id'=>$row['id'],'type'=>$type));?>';" title="修改" class="btn btn-info btn-smsm"><i class="fa fa-pencil"></i></button>
                                            </td><?php endif; ?>-->
                                        <?php if(rolemenu('Res/res_del')): ?><td class="taskOptions">

                                                <a onClick="javascript:ConfirmDel('<?php echo U('Res/res_del',array('id'=>$row['id']));?>');" title="删除" class="btn btn-warning btn-smsm" style="color:#ffffff;"><i class="fa fa-times"></i></a>
                                            </td><?php endif; ?>
...
<div id="searchtext">
    <form action="<?php echo U('Res/index');?>" method="get" id="searchform">
        ...
        <div class="form-group col-md-6">
            <select class="form-control" name="type">
                ...
                <?php if(is_array($kind)): foreach($kind as $k=>$v): ?><option value="<?php echo ($k); ?>"><?php echo ($v); ?></option><?php endforeach; endif; ?>
            </select>
        </div>
        <div class="form-group col-md-6">
