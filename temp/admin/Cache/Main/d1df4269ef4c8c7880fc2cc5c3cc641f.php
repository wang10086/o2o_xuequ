    ...
    <aside class="right-side">
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li><a href="<?php echo U('Expert/index',array('type'=>$type));?>"><?php echo ($pagename); ?>管理</a></li>
            </ol>
        </section>
                ...
                <div class="col-md-12">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title"><?php echo ($pagename); ?>列表</h3>
                            <div class="pull-right box-tools">

                                <button class="btn btn-info btn-sm" onclick="javascript:opensearch('searchtext',700,100);"><i class="fa fa-search"></i> 搜索</button>
                                <?php if(rolemenu('Expert/add')): ?><a href="<?php echo U('Expert/add',array('kind'=>$kind));?>" style="color:#ffffff;" class="btn btn-danger btn-sm"><i class="fa fa-plus"></i> 发布文章</a><?php endif; ?>
                            </div>
                        </div><!-- /.box-header -->
                        <div class="box-body" id="piclist">
                            <table class="table table-bordered dataTable fontmini" id="tablelist">
                                <tr role="row" class="orders" >
                                    ...
                                    <?php if(rolemenu('Expert/edit')): ?><th width="60" class="taskOptions">编辑</th><?php endif; ?>
                                    <?php if($type==1){ ?>
                                        <?php if(rolemenu('Delete/del_news')): ?><th width="60" class="taskOptions">删除</th><?php endif; ?>
                                    <?php }else{ ?>
                                        <?php if(rolemenu('Delete/del_subject')): ?><th width="60" class="taskOptions">删除</th><?php endif; ?>
                                    <?php } ?>
                                </tr>
                                <?php if(is_array($infolist)): foreach($infolist as $key=>$row): ?><tr>
                                        <td class="taskOptions" style="line-height:24px;">
                                            <a href="<?php echo U('Attachment/order',array('id'=>$row['id'],'type'=>$type));?>" title="置顶" class="btn btn-success btn-smsm"><i class="fa fa-upload"></i></a>
                                        </td>

                                        <td style="line-height:24px;"><?php echo ($row["sort"]); ?></td>
                                        <td style="line-height:24px;"><?php echo ($row["id"]); ?></td>
                                        <td style="line-height:24px;">
                                            <?php echo ($row["pic"]); ?>
                                            <?php
 $con = C('TMPL_PARSE_STRING'); if($kind==1){ $url = $con['__HTTP_URL__'].'index.php?m=Main&c=Expert&a=detail&id='.$row['id']; }else if($kind==2){ $url = $con['__HTTP_URL__'].'index.php?m=Main&c=Collection&a=detail&id='.$row['id']; }else{ $url = 'javascript:;'; } ?>
                                            <a href="<?php echo ($url); ?>" target="_blank"><?php echo ($row["title"]); ?></a>

                                        </td>
                                        <td style="line-height:24px;"><?php echo ($row["editor"]); ?></td>
                                        <td style="line-height:24px;"><?php echo ($row["source"]); ?></td>
                                        <td style="line-height:24px;"><?php if($row['create_time']): echo (date('Y-m-d H:i',$row["create_time"])); endif; ?></td>
                                        <td style="line-height:24px;"><?php if($row['update_time']): echo (date('Y-m-d H:i',$row["update_time"])); endif; ?></td>
                                        <?php if(rolemenu('Expert/edit')): ?><td class="taskOptions"  style="line-height:24px;">
                                                <a href="<?php echo U('Expert/edit',array('id'=>$row['id'],'kind'=>$kind));?>" title="修改" class="btn btn-info btn-smsm"><i class="fa fa-pencil"></i></a>
                                            </td><?php endif; ?>

                                        <?php if(rolemenu('Expert/del_news')): ?><td class="taskOptions" style="line-height:24px;">
                                                <a onClick="javascript:ConfirmDel('<?php echo U('Expert/del_news',array('id'=>$row['id']));?>');" title="删除" class="btn btn-warning btn-smsm"><i class="fa fa-times"></i></a>
                                            </td><?php endif; ?>
...
<div id="searchtext">
    <form method="get" action="<?php echo U('Expert/index');?>" id="searchfrom" name="myform">
        <input type="hidden" name="m" value="Main">
        <input type="hidden" name="c" value="Expert">
        ...
        <div class="form-group col-md-6">
            <select class="form-control" name="col">
                ...
                <?php if(is_array($infotype)): foreach($infotype as $k=>$v): ?><option value="<?php echo ($v["id"]); ?>"><?php echo ($v["title"]); ?></option><?php endforeach; endif; ?>
            </select>
        </div>
