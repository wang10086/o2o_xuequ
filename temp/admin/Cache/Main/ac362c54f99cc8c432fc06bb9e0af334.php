                            ...
                            <div class="box">
                                <div class="box-header">
                                    ...
                                    <div class="pull-right box-tools">
                                        ...
                                        <?php if(rolemenu(array('User/add'))): ?><button onClick="javascript:window.location.href='<?php echo U('User/add');?>';" class="btn btn-danger btn-sm"><i class="fa fa-plus"></i></button><?php endif; ?>
                                    </div>
                                </div><!-- /.box-header -->
                                <div class="box-body">
                                    <table class="table table-bordered dataTable fontmini" id="tablelist">
                                        <tr role="row" class="orders" >
                                        	<?php if(rolemenu(array('User/status'))): ?><th width="60" class="taskOptions sorting" data="a.status">状态</th><?php endif; ?>
                                            ...
                                            <?php if(rolemenu(array('User/password'))): ?><th width="60" class="taskOptions">密码</th><?php endif; ?>
                                            <?php if(rolemenu(array('User/edit'))): ?><th width="60" class="taskOptions">编辑</th><?php endif; ?>
                                            <?php if(rolemenu(array('Delete/user'))): ?><th width="60" class="taskOptions">删除</th><?php endif; ?>
                                        </tr>
                                        <?php if(is_array($users)): foreach($users as $key=>$row): ?><tr>
                                        	<?php if(rolemenu(array('User/status'))): ?><td class="taskOptions" style="line-height:32px;">
                                            <?php if($row['status']==0){ echo '<span class="green">正常</span>'; }else{ echo '<span class="red">停用</span>'; } ?>
                                            </td><?php endif; ?>
                                            <td align="center" style="text-align:center;"><img src="<?php echo ($row["avatar"]); ?>" class="useravatar"></td>
                                            <td style="line-height:32px;"><?php echo ($row["username"]); ?></td>
                                            <td style="line-height:32px;"><?php echo ($row["name"]); ?></td>
                                            <td style="line-height:32px;"><?php echo ($row["mobile"]); ?></td>
                                            <td style="line-height:32px;"><?php echo ($row["email"]); ?></td>
                                            <td style="line-height:32px;"><?php if($row['update_time']): echo (date('Y-m-d H:i:s',$row["update_time"])); endif; ?></td>
                                            <td style="line-height:32px;"><?php echo ($row["remark"]); ?></td>
                                            <?php if(rolemenu(array('User/password'))): ?><td class="taskOptions" style="line-height:32px;">
                                            <button onClick="javascript:window.location.href='<?php echo U('User/password',array('id'=>$row['userid']));?>';" title="修改密码" class="btn btn-success btn-smsm"><i class="fa fa-lock"></i></button>
                                            </td><?php endif; ?>
                                            <?php if(rolemenu(array('User/edit'))): ?><td class="taskOptions" style="line-height:32px;">
                                            <button onClick="javascript:window.location.href='<?php echo U('User/edit',array('id'=>$row['userid']));?>';" title="修改资料" class="btn btn-info btn-smsm"><i class="fa fa-pencil"></i></button>
                                            </td><?php endif; ?>
                                            <?php if(rolemenu(array('Delete/user'))): ?><td class="taskOptions" style="line-height:32px;">
                                            <?php  if($row['username']!=C('RBAC_SUPER_ADMIN')){ ?>
                                            <button onClick="javascript:ConfirmDel('<?php echo U('Delete/user',array('id'=>$row['userid']));?>');" title="删除用户" class="btn btn-warning btn-smsm"><i class="fa fa-times"></i></button>
                                            <?php } ?>
                                            </td><?php endif; ?>
        ...
        <div id="searchtext">
            ...
            <div class="form-group">
                <select  class="form-control"  name="pid">
                    ...
                    <?php if(is_array($companylist)): foreach($companylist as $key=>$row): ?><option value="<?php echo ($row["id"]); ?>" <?php if($pid==$row['id']){ echo 'selected';} ?>><?php echo ($row["company_name"]); ?></option><?php endforeach; endif; ?>
                </select>
            </div>
            <div class="form-group">
                <select  class="form-control"  name="role">
                    <option value="">角色</option>
                    <?php if(is_array($rolelist)): foreach($rolelist as $key=>$row): ?><option value="<?php echo ($row["id"]); ?>" <?php if($role==$row['id']){ echo 'selected';} ?>><?php echo ($row["remark"]); ?></option><?php endforeach; endif; ?>       
                </select>
            </div>
            <div class="form-group">
                <input type="text" name="user" placeholder="用户名" value="<?php echo ($user); ?>" class="form-control">
            </div>
            <div class="form-group">
                <input type="text" name="name" placeholder="使用者" value="<?php echo ($name); ?>" class="form-control"/>
            </div>
            </form>
        </div>
