		...
		for (var i=startDay; i<=endDay; i++) {
			...
			//if(role < 4){
				if (dayCounter==day) {
					...
					$("#calBox" + i).html("<div class='date' id='" + year + "-" + tempmonth + "-" + tempday + "' onclick='openAddBox(\""+jin+"\",0,"+pid+")'>" + dayCounter + "</div>");
				}
			//}else{
			//	$("#calBox" + i).html("<div class='nodate' id='" + year + "-" + tempmonth + "-" + tempday + "'>" + dayCounter + "</div>");
			//}
	...
	//从服务器获取任务信息
	function getTasks() {
		  $.post(web_path+'op.php?m=Main&c=Plan&a=public_schedule',{pid:pid,month: year + "-" + (month +1)},function(e){
			if(e != null){
			 $(e).each(function(i){
	...
	function Taskinfo(id,pid,status) {
		...
		//if(role < 4){
			...
			//课堂记录
			var btn_record = {name:'记录',callback:function(){ record(id); },focus:false}
			
			//开课
			var btn_start = {name:'签到',callback:function(){ startTask(id); },focus:false}
			...
			if(status == 0){
				...
				var btns = [btn_record,btn_start];	
			}else if(status==20){
				var btns = [];		
			}else if(status==30){
				var btns = [];		
			}else if(status==40){
				var btns = [btn_re,btn_del];		
			}
